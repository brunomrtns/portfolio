  ...
  const config = {
    ...
    events: {
      "image.beforeUpload": function (images: File[]) {
        const editor = this as any; // Acesso ao editor Froala
        const formData = new FormData();
        formData.append("file", images[0]); // Froala envia o primeiro arquivo selecionado
        ...
        axios
          ...
          .then((response) => {
            const imageUrl = response.data.link; // Froala espera o campo `link`
            if (imageUrl) {
              editor.image.insert(imageUrl, true); // Insere a imagem no editor
            } else {
              console.error("No link field in response:", response.data);
            }
          })
          .catch((error) => {
        ...
        // Cancela o upload padrão do Froala
        return false;
      },
    },
  };
